...
final class ErrorAnalytics {
	...
	public static function init(): void {
		...
		add_action( 'nuclen_cleanup_analytics_data', [ self::class, 'cleanup_analytics_data' ] );
	}
	...
	public static function track_error( array $error_data ): void {
		...
		if ( count( self::$error_tracking[ $key ]['examples'] ) < 5 ) {
			self::$error_tracking[ $key ]['examples'][] = [
				'id' => $error_data['id'],
	...
	public static function cleanup_analytics_data(): void {
		...
		foreach ( self::$error_tracking as $key => $data ) {
			// Remove old examples
			$data['examples'] = array_filter( $data['examples'], function( $example ) use ( $cutoff ) {
	...
	public static function load_tracking_data(): void {
		self::$error_tracking = get_option( 'nuclen_error_tracking', [] );
		self::$rate_limits = get_option( 'nuclen_rate_limits', [] );
	}
