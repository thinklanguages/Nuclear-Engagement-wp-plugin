...
final class QueryOptimizer {
	...
	/**
	 * Get slow query threshold from config or use default.
	 */
	public static function get_slow_query_threshold(): float {
		return defined( 'NUCLEN_SLOW_QUERY_THRESHOLD' ) ? (float) NUCLEN_SLOW_QUERY_THRESHOLD : self::SLOW_QUERY_THRESHOLD;
	}
	...
	public static function get_batch_size(): int {
		return defined( 'NUCLEN_BATCH_SIZE' ) ? (int) NUCLEN_BATCH_SIZE : self::BATCH_SIZE;
	}

	/**
	 * Get max batch size from config or use default.
	 */
	public static function get_max_batch_size(): int {
		return defined( 'NUCLEN_MAX_BATCH_SIZE' ) ? (int) NUCLEN_MAX_BATCH_SIZE : self::MAX_BATCH_SIZE;
	}
	...
	public static function init(): void {
		...
		add_filter( 'posts_pre_query', [ self::class, 'optimize_wp_query' ], 10, 2 );
		
		// Track slow queries if debug mode is enabled
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			add_action( 'shutdown', [ self::class, 'log_slow_queries' ] );
		}
	}
	 ...
	 */
	public static function get_posts_metadata_bulk( array $post_ids, array $meta_keys = [] ): array {
		if ( empty( $post_ids ) ) {
			return [];
		}

		$cache_key = 'bulk_metadata_' . md5( serialize( $post_ids ) . serialize( $meta_keys ) );
		
		return CacheManager::remember( $cache_key, function() use ( $post_ids, $meta_keys ) {
			global $wpdb;
			...
			return $metadata;
		}, 'metadata', 1800 ); // 30 minutes cache
	}
	 ...
	 */
	public static function get_posts_with_metadata( array $args, array $meta_keys = [] ): array {
		$cache_key = 'posts_with_meta_' . md5( serialize( $args ) . serialize( $meta_keys ) );
		
		return CacheManager::remember( $cache_key, function() use ( $args, $meta_keys ) {
			...
			$batch_size = empty( $meta_keys ) ? self::get_max_batch_size() : self::get_batch_size();
	...
	public static function get_quiz_summary_data( array $post_ids ): array {
		...
		$cache_key = 'quiz_summary_' . md5( serialize( $post_ids ) );
		
		return CacheManager::remember( $cache_key, function() use ( $post_ids ) {
			global $wpdb;

			$placeholders = implode( ',', array_fill( 0, count( $post_ids ), '%d' ) );
			$params = array_merge( $post_ids, $post_ids );

			// Check if indexes exist for optimization
			$has_indexes = self::check_indexes_exist();
			
			if ( $has_indexes && count( $post_ids ) > 100 ) {
				// Use optimized query with index hints for large datasets
				$sql = $wpdb->prepare( "
					SELECT /*+ INDEX(pm_quiz post_id) INDEX(pm_summary post_id) */
						p.ID,
						p.post_title,
						p.post_type,
						p.post_status,
				...
				// Use simpler query for smaller datasets or when indexes are missing
				$sql = $wpdb->prepare( "
					SELECT 
						p.ID,
						p.post_title,
			...
			foreach ( $results as $row ) {
				$data[ $row->ID ] = [
					...
					'quiz_data'    => $row->quiz_data ? json_decode( $row->quiz_data, true ) : null,
					'summary_data' => $row->summary_data ? json_decode( $row->summary_data, true ) : null,
	...
	public static function get_dashboard_stats(): array {
		return CacheManager::remember( 'dashboard_stats', function() {
			global $wpdb;
	 ...
	 */
	public static function execute_cached_query( string $sql, array $params = [], string $cache_key = '', int $cache_ttl = 300, string $cache_group = 'queries' ) {
		if ( empty( $cache_key ) ) {
			$cache_key = 'query_' . md5( $sql . serialize( $params ) );
		}

		return CacheManager::remember( $cache_key, function() use ( $sql, $params ) {
			global $wpdb;
	...
	private static function track_query_performance( string $query_type, float $query_time ): void {
		...
		$threshold = self::get_slow_query_threshold();
		if ( $query_time > $threshold ) {
			$stats['slow_queries']++;
		}

		// Log slow queries immediately
		if ( $query_time > $threshold && defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			error_log( sprintf( 
	...
	public static function log_slow_queries(): void {
		$slow_queries = array_filter( self::$query_stats, function( $stats ) {
			return $stats['slow_queries'] > 0;
		} );

		if ( ! empty( $slow_queries ) ) {
			error_log( 'Nuclear Engagement Query Performance Summary: ' . json_encode( $slow_queries ) );
		}
	}
	...
	/**
	 * Check if database indexes exist for optimization.
	 *
	 * @return bool Whether indexes exist.
	 */
	private static function check_indexes_exist(): bool {
		static $indexes_exist = null;
		
		if ( $indexes_exist !== null ) {
			return $indexes_exist;
		}

		global $wpdb;
		
		// Check for composite index on postmeta
		$index_check = $wpdb->get_var( $wpdb->prepare(
			"SHOW INDEX FROM {$wpdb->postmeta} WHERE Key_name = %s",
			'nuclen_post_meta_key'
		) );
		
		$indexes_exist = ! empty( $index_check );
	...
	public static function create_indexes(): void {
		...
		// Add composite index for our meta queries if not exists
		$wpdb->query( "
			CREATE INDEX IF NOT EXISTS nuclen_post_meta_key 
			ON {$wpdb->postmeta} (post_id, meta_key(50))
		" );
	}
	...
	public static function warmup_queries(): void {
		...
		if ( ! empty( $recent_posts ) ) {
			self::get_posts_metadata_bulk( $recent_posts, [
				'nuclen-quiz-data',
				'nuclen-summary-data',
			] );
		}
