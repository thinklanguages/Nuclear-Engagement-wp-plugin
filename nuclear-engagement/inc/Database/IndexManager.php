...
class IndexManager {
	...
	public static function remove_performance_indexes(): void {
		...
		$tables_and_indexes = [
			$wpdb->posts => ['ne_post_status_type_date', 'ne_post_type_status_modified'],
			$wpdb->postmeta => ['ne_meta_key_value', 'ne_post_meta_key_value'],
			$wpdb->usermeta => ['ne_user_meta_key_value'],
			$wpdb->options => ['ne_option_name_autoload']
		];
	...
	public static function analyze_table_performance(): array {
		...
		$analysis = [];
		$tables = [ $wpdb->posts, $wpdb->postmeta, $wpdb->usermeta, $wpdb->options ];
