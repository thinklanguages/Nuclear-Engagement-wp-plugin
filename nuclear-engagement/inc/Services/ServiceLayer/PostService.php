...
class PostService {
	...
	public function get_post( int $post_id ): ?Post {
		...
		try {
			...
			$this->logger->error( 'Failed to get post', array(
				'post_id' => $post_id,
				'error' => $e->getMessage(),
			) );
			return null;
	...
	public function find_posts_for_generation( string $workflow_type, array $criteria = array(), int $limit = 100 ): array {
		...
		$this->logger->debug( 'Finding posts for generation', array(
		...
		try {
			...
			$event = new Event( 'nuclen.posts.found_for_generation', array(
				'workflow_type' => $workflow_type,
				'posts' => $posts,
				'count' => count( $posts ),
			) );
			...
			$this->logger->error( 'Failed to find posts for generation', array(
				'workflow_type' => $workflow_type,
				'criteria' => $criteria,
				'error' => $e->getMessage(),
			) );
	...
	public function update_post_meta( int $post_id, string $meta_key, $meta_value ): bool {
		...
		$this->logger->debug( 'Updating post meta', array(
			'post_id' => $post_id,
			'meta_key' => $meta_key,
		) );
		...
		try {
			...
			if ( $result ) {
				$event = new Event( 'nuclen.post.meta_updated', array(
					'post_id' => $post_id,
					'meta_key' => $meta_key,
					'old_value' => $old_value,
			...
			$this->logger->error( 'Failed to update post meta', array(
				'post_id' => $post_id,
				'meta_key' => $meta_key,
				'error' => $e->getMessage(),
			) );
	...
	public function protect_post( int $post_id, string $workflow_type ): bool {
		...
		if ( $result ) {
			$this->logger->info( 'Post protected from generation', array(
				'post_id' => $post_id,
				'workflow_type' => $workflow_type,
			) );
			
			$event = new Event( 'nuclen.post.protected', array(
				'post_id' => $post_id,
				'workflow_type' => $workflow_type,
			) );
			$this->event_dispatcher->dispatch( $event );
	...
	public function unprotect_post( int $post_id, string $workflow_type ): bool {
		...
		if ( $result ) {
			$this->logger->info( 'Post unprotected from generation', array(
				'post_id' => $post_id,
				'workflow_type' => $workflow_type,
			) );
			
			$event = new Event( 'nuclen.post.unprotected', array(
				'post_id' => $post_id,
				'workflow_type' => $workflow_type,
			) );
			$this->event_dispatcher->dispatch( $event );
	...
	public function get_statistics( array $criteria = array() ): array {
		...
		try {
			...
			$event = new Event( 'nuclen.post.statistics_calculated', array(
			...
			$this->logger->error( 'Failed to get post statistics', array(
				'criteria' => $criteria,
				'error' => $e->getMessage(),
			) );
			return array();
