...
abstract class DatabaseRepository {
	 ...
	 */
	protected function execute_query( string $query, array $params = [] ) {
		if ( empty( $params ) ) {
	 ...
	 */
	protected function get_row( string $query, array $params = [], string $output = OBJECT ) {
		if ( empty( $params ) ) {
	 ...
	 */
	protected function get_results( string $query, array $params = [], string $output = OBJECT ): array {
		if ( empty( $params ) ) {
	 ...
	 */
	protected function get_var( string $query, array $params = [], int $col_offset = 0, int $row_offset = 0 ) {
		if ( empty( $params ) ) {
	...
	protected function table_exists( string $table_name ): bool {
		...
		$result = $this->get_var( $query, [ $table_name ] );
