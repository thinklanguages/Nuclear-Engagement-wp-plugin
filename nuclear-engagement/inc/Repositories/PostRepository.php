<?php
/**
 * PostRepository.php - Part of the Nuclear Engagement plugin.
 *
...
class PostRepository extends AbstractRepository {
	...
	public function find_with_meta( array $criteria = array(), array $meta_criteria = array(), array $order_by = array(), int $limit = 0, int $offset = 0 ): array {
		$cache_key = $this->cache_key( 'find_with_meta_' . md5( maybe_serialize( func_get_args() ) ) );
		...
		// Load full post objects in batch to avoid N+1 queries.
		$posts = array();
		if ( ! empty( $post_ids ) ) {
			// Query all posts in a single batch operation.
			$placeholders = implode( ',', array_fill( 0, count( $post_ids ), '%d' ) );
			$sql          = $this->get_wpdb()->prepare(
				"SELECT * FROM {$this->get_table_name()} WHERE ID IN ($placeholders)",
				$post_ids
			);

			$rows = $this->get_wpdb()->get_results( $sql );
			...
			if ( ! empty( $rows ) ) {
				foreach ( $rows as $row ) {
					...
					// Cache individual posts for future single lookups.
					$this->cache->set( $this->cache_key( (string) $post->get_id() ), $post, $this->cache_ttl );
				}
			}
		}
	...
	public function count_with_meta( array $criteria = array(), array $meta_criteria = array() ): int {
		$cache_key = $this->cache_key( 'count_with_meta_' . md5( maybe_serialize( func_get_args() ) ) );
		$cached    = $this->cache->get( $cache_key );
