...
final class QueryOptimizer {
	 ...
	 */
	public function query(string $sql, array $params = [], int $cache_ttl = self::CACHE_TTL): array {
		global $wpdb;
		...
		if ($execution_time > 1.0) { // > 1 second
			error_log("Slow query detected ({$execution_time}s): {$prepared_sql}");
		}
	 ...
	 */
	public function queryRow(string $sql, array $params = [], int $cache_ttl = self::CACHE_TTL): ?array {
	 ...
	 */
	public function queryVar(string $sql, array $params = [], int $cache_ttl = self::CACHE_TTL) {
		global $wpdb;
	 ...
	 */
	public function batchInsert(string $table, array $data, array $format = []): bool {
		global $wpdb;
