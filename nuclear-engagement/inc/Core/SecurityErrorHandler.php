...
final class SecurityErrorHandler {
	 ...
	 */
	private static array $security_thresholds = [
	 ...
	 */
	private static array $blocked_ips = [];
	 ...
	 */
	private static array $suspicious_patterns = [
		'sql_injection' => [
	...
	public static function init(): void {
		...
		add_action( 'wp_login', [ self::class, 'handle_successful_login' ], 10, 2 );
		add_filter( 'authenticate', [ self::class, 'check_ip_before_auth' ], 1, 3 );

		// Monitor specific WordPress events
		add_action( 'wp_ajax_nopriv_*', [ self::class, 'monitor_ajax_requests' ], 1 );
		add_action( 'wp_ajax_*', [ self::class, 'monitor_ajax_requests' ], 1 );
		add_action( 'rest_api_init', [ self::class, 'monitor_rest_requests' ] );

		// Clean up old security data
		if ( ! wp_next_scheduled( 'nuclen_cleanup_security_data' ) ) {
			wp_schedule_event( time(), 'hourly', 'nuclen_cleanup_security_data' );
		}
		add_action( 'nuclen_cleanup_security_data', [ self::class, 'cleanup_security_data' ] );

		// Register shutdown handler to save security data
		register_shutdown_function( [ self::class, 'save_security_data' ] );
	}
	...
	public static function detect_suspicious_activity( array $request_data ): bool {
		...
		if ( $suspicious_detected ) {
			self::handle_security_error( self::EVENT_SUSPICIOUS_ACTIVITY, [
	...
	public static function apply_rate_limit( string $identifier, int $limit = 60, int $window = 3600 ): bool {
		...
		if ( $rate_data['count'] > $limit ) {
			...
			self::handle_security_error( self::EVENT_RATE_LIMIT_EXCEEDED, [
	...
	public static function sanitize_input( $input ) {
		...
		foreach ( self::$suspicious_patterns as $category => $patterns ) {
			foreach ( $patterns as $pattern ) {
				if ( preg_match( $pattern, $input ) ) {
					self::handle_security_error( self::EVENT_INJECTION_ATTEMPT, [
						'category' => $category,
		...
		// Sanitize the input
		$sanitized = wp_kses( $input, [] ); // Remove all HTML
	...
	public static function get_security_dashboard( int $time_window = 86400 ): array {
		...
		foreach ( self::$security_events as $event_type => $events ) {
			$recent_events = array_filter( $events, function( $event ) use ( $cutoff ) {
	...
	public static function handle_login_failure( string $username ): void {
		...
		self::handle_security_error( self::EVENT_BRUTE_FORCE, [
			'username' => $username,
			'failed_login' => true,
		] );
	}
