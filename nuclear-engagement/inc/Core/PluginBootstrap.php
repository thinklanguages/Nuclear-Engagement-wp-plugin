...
final class PluginBootstrap {
	...
	private function registerWordPressHooks(): void {
		...
		add_action('init', [$this, 'registerAutoGenerationHooks'], 10);
	...
	private function initializeAdminServices(): void {
		...
		if (class_exists('NuclearEngagement\Admin\Admin')) {
			...
			add_action('init', [$admin, 'nuclen_register_admin_scripts'], 9);
			add_action('admin_enqueue_scripts', [$admin, 'wp_enqueue_styles']);
			add_action('admin_enqueue_scripts', [$admin, 'wp_enqueue_scripts']);
			add_action('admin_enqueue_scripts', [$admin, 'nuclen_enqueue_dashboard_styles']);
			add_action('admin_enqueue_scripts', [$admin, 'nuclen_enqueue_generate_page_scripts']);
	...
	private function registerAdminAjaxHooks(ServiceContainer $container): void {
		...
		add_action('wp_ajax_nuclen_trigger_generation', [$generate_controller, 'handle']);
		add_action('wp_ajax_nuclen_fetch_app_updates', [$updates_controller, 'handle']);
		add_action('wp_ajax_nuclen_get_posts_count', [$posts_count_controller, 'handle']);
		add_action('wp_ajax_nuclen_dismiss_pointer', [$pointer_controller, 'dismiss']);
	}
	
	private function registerSetupHooks(SettingsRepository $settings): void {
		$setup = new \NuclearEngagement\Admin\Setup($settings);
		add_action('admin_post_nuclen_connect_app', [$setup, 'nuclen_handle_connect_app']);
		add_action('admin_post_nuclen_generate_app_password', [$setup, 'nuclen_handle_generate_app_password']);
		add_action('admin_post_nuclen_reset_api_key', [$setup, 'nuclen_handle_reset_api_key']);
		add_action('admin_post_nuclen_reset_wp_app_connection', [$setup, 'nuclen_handle_reset_wp_app_connection']);
	}
	...
	private function registerExportHooks(ServiceContainer $container): void {
		...
		add_action('admin_post_nuclen_export_optin', [$optin_export_controller, 'handle']);
		add_action('wp_ajax_nuclen_export_optin', [$optin_export_controller, 'handle']);
	}
	...
	private function initializeFrontendServices(): void {
		...
		if (class_exists('NuclearEngagement\Front\FrontClass')) {
			...
			add_action('wp_head', [$frontend, 'wp_head_custom_theme_vars'], 99);
			add_action('init', [$frontend, 'nuclen_register_quiz_shortcode']);
			add_action('init', [$frontend, 'nuclen_register_summary_shortcode']);
			add_filter('the_content', [$frontend, 'nuclen_auto_insert_shortcodes'], 50);
	...
	private function initializeApiServices(): void {
		...
		if (class_exists('NuclearEngagement\Front\Controller\Rest\ContentController')) {
			...
			register_rest_route(
				...
				[
					'methods' => 'POST',
					'callback' => [$content_controller, 'handle'],
					'permission_callback' => [$content_controller, 'permissions'],
				]
			);
		}
	}
