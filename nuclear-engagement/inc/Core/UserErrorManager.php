<?php
/**
 * UserErrorManager.php - Part of the Nuclear Engagement plugin.
 *
...
final class UserErrorManager {
	 ...
	 */
	private static array $notification_preferences = array();
	 ...
	 */
	private static array $message_templates = array(
		ErrorManager::CATEGORY_AUTHENTICATION => array(
		...
		ErrorManager::CATEGORY_DATABASE       => array(
		...
		ErrorManager::CATEGORY_VALIDATION     => array(
		...
		ErrorManager::CATEGORY_PERMISSIONS    => array(
		...
		ErrorManager::CATEGORY_NETWORK        => array(
		...
		ErrorManager::CATEGORY_EXTERNAL_API   => array(
	 ...
	 */
	private static array $suggested_actions = array(
		ErrorManager::CATEGORY_AUTHENTICATION => array(
	 ...
	 * @var array<int, array>
	 */
	private static array $user_error_history = array();
	 ...
	 * @var array<string, array>
	 */
	private static array $resolution_tracking = array();
	...
	public static function init(): void {
		...
		add_action( 'admin_notices', array( self::class, 'display_admin_error_notices' ) );
		add_action( 'wp_ajax_nuclen_dismiss_error', array( self::class, 'handle_error_dismissal' ) );
		add_action( 'wp_ajax_nuclen_resolve_error', array( self::class, 'handle_error_resolution' ) );
		add_action( 'wp_ajax_nuclen_get_error_help', array( self::class, 'provide_error_help' ) );

		// Frontend error handling.
		add_action( 'wp_footer', array( self::class, 'inject_error_handler_script' ) );
		add_action( 'wp_ajax_nopriv_nuclen_report_frontend_error', array( self::class, 'handle_frontend_error_report' ) );
		add_action( 'wp_ajax_nuclen_report_frontend_error', array( self::class, 'handle_frontend_error_report' ) );

		// Load user preferences.
		add_action( 'wp_login', array( self::class, 'load_user_error_preferences' ), 10, 2 );

		// Clean up old error data.
		if ( ! wp_next_scheduled( 'nuclen_cleanup_user_errors' ) ) {
			wp_schedule_event( time(), 'daily', 'nuclen_cleanup_user_errors' );
		}
		add_action( 'nuclen_cleanup_user_errors', array( self::class, 'cleanup_user_error_data' ) );
	}
	...
	public static function get_user_friendly_message( ErrorContext $error_context, bool $is_admin = false ): string {
		...
		$user_context         = $error_context->get_context_value( 'user_context', array() );
	...
	public static function get_suggested_actions( ErrorContext $error_context ): array {
		$category = $error_context->get_category();
		$actions  = self::$suggested_actions[ $category ] ?? array();
	...
	public static function handle_error_dismissal(): void {
		...
		if ( $error_id ) {
			...
			wp_send_json_success( array( 'message' => 'Error dismissed successfully' ) );
		}

		wp_send_json_error( array( 'message' => 'Invalid error ID' ) );
	}
	...
	public static function handle_error_resolution(): void {
		...
		if ( $error_id && $resolution_method ) {
			...
			wp_send_json_success( array( 'message' => 'Thank you for the feedback' ) );
		}

		wp_send_json_error( array( 'message' => 'Invalid resolution data' ) );
	}
	...
	public static function provide_error_help(): void {
		...
		wp_send_json_success( array( 'help_content' => $help_content ) );
	}
	...
	public static function handle_frontend_error_report(): void {
		$error_data   = $_POST['error_data'] ?? array();
		$user_context = $_POST['user_context'] ?? array();
		...
		wp_send_json_success( array( 'error_id' => $error_context->get_error_id() ) );
	}
