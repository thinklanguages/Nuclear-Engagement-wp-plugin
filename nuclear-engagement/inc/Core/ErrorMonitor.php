...
final class ErrorMonitor {
	 ...
	 */
	private static array $notification_thresholds = [
		'critical' => [ 'threshold' => 1, 'interval' => 300 ],     // Immediate
		'high' => [ 'threshold' => 5, 'interval' => 600 ],         // 5 in 10 min
		'medium' => [ 'threshold' => 10, 'interval' => 1800 ],     // 10 in 30 min
		'low' => [ 'threshold' => 20, 'interval' => 3600 ],        // 20 in 1 hour
	];
	...
	public static function init(): void {
		...
		add_action( 'nuclen_cleanup_error_data', [ self::class, 'cleanup_old_data' ] );
		...
		add_action( 'nuclen_generate_error_analytics', [ self::class, 'generate_analytics' ] );
	}
	...
	public static function cleanup_old_data(): void {
		...
		$old_reports = get_option( 'nuclen_error_analytics_reports', [] );
		$cleaned_reports = array_filter( $old_reports, function( $report ) use ( $cutoff ) {
	...
	private static function store_error_stat( ErrorContext $error_context ): void {
		...
		$wpdb->insert(
			...
			[
				...
				'category' => $error_context->get_category(),
				'severity' => $error_context->get_severity(),
				'context' => json_encode( $error_context->get_context() ),
				'timestamp' => time(),
			],
			[ '%s', '%s', '%s', '%s', '%d' ]
		);
	...
	private static function maybe_create_error_tables(): void {
		...
		if ( $wpdb->get_var( "SHOW TABLES LIKE '{$error_log_table}'" ) !== $error_log_table ) {
	...
	private static function determine_security_event_type( ErrorContext $context ): string {
		...
		if ( strpos( $message, 'login' ) !== false ) return 'failed_login';
		if ( strpos( $message, 'permission' ) !== false ) return 'permission_denied';
		if ( strpos( $message, 'injection' ) !== false ) return 'injection_attempt';
	...
	private static function format_notification_message( ErrorContext $context, string $type ): string {
		...
		if ( ! empty( $context_data ) ) {
			$message .= "<h3>Context</h3>\n<pre>" . esc_html( json_encode( $context_data, JSON_PRETTY_PRINT ) ) . "</pre>\n";
		}
	...
	private static function generate_summary( array $error_stats, array $security_stats ): array {
		...
		return [
			...
			'critical_errors' => count( array_filter( $error_stats, function( $stat ) {
