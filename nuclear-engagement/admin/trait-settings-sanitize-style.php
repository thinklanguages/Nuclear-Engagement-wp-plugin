 ...
 *
 * Sanitises every visual-style field for quiz & summary.
 *
...
trait SettingsSanitizeStyleTrait {
    ...
    /**
     * Validate an HTML-hex colour (3/4/6/8‑digit) *or* return an empty string.
     *
     * • 3/4‑digit codes are expanded to 6/8‑digit formats.
     *
     * This helper is kept *private* because we now allow additional CSS colour
     * formats (rgb / rgba) in the outer wrapper (see `$hx()` below). Keeping
     * this strict avoids unintended acceptance of malformed strings.
     */
    private static function nuclen_validate_hex( string $raw ): string {
        ...
        // #RRGGBB or #RRGGBBAA → keep as‑is (lower‑cased)
    ...
    /**
     * Sanitise all style‑related keys.
     *
     * We now accept *both* HEX and `rgb()` / `rgba()` strings so that the admin
     * colour‑picker’s **Alpha** slider (which returns `rgba( … )`) is preserved
     * correctly. If the value is neither a valid hex **nor** a valid `rgb/rgba`
     * pattern we fall back to the provided default.
     *
     * @param array $in Raw settings.
     * @return array    Clean style keys.
     */
    private function nuclen_sanitize_style( array $in ): array {

        /* ------------------------------------------------------------
         * Helper: validate colour or fall back
         * ---------------------------------------------------------- */
        $hx = static function ( $val, $fallback ) {
            $val = (string) $val;

            /* 1) HEX (#fff, #ffffff, #ffffffff) */
            $hex = self::nuclen_validate_hex( $val );
            if ( $hex !== '' ) {
                return $hex;
            }

            /* 2) rgb() / rgba() – allow 0‑255 vals + optional alpha 0‑1 */
            $rgb_regex = '/^rgba?\(\s*(?:\d{1,3}\s*,\s*){2}\d{1,3}(?:\s*,\s*(?:0|1|0?\.\d+))?\s*\)$/i';
            if ( preg_match( $rgb_regex, $val ) ) {
                return $val; // already safe, just trim
            }
