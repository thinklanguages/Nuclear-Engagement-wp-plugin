...
export function initGeneratePage(): void {
  ...
  generateForm?.addEventListener('submit', async (event) => {
    ...
    if (updatesContent) {
      updatesContent.innerText = 'Processing posts... Do NOT leave this page until the process is complete.';
    }
    nuclenHideElement(step2);
    if (submitBtn) {
      submitBtn.disabled = true;
        ...
        onComplete: async ({ failCount, finalReport, results, workflow }) => {
          ...
          if (updatesContent) {
            if (failCount && finalReport) {
              ...
              updatesContent.innerText = 'All posts processed successfully! Your content has been saved.';
